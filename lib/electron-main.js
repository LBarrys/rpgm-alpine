'use strict';
// Electron backend: runs an NW.js game (RPG Maker MV/MZ) with Alpine's musl-native Electron.
...
// Run from the folder holding index.html (www/ for MV), which is the base NW.js games
// assume. Mod loaders resolve www/mods and www/js/loaders from here; using the game
// root instead makes them search one level too high and silently find nothing.
...
app.commandLine.appendSwitch('js-flags', '--expose-gc'); // MV's package.json asks for this
app.commandLine.appendSwitch('ignore-gpu-blocklist'); // keep WebGL on older GPUs, as NW.js games did
process.env.ELECTRON_DISABLE_SECURITY_WARNINGS = 'true'; // webSecurity is off on purpose (NW.js parity)

// Serve the game from app://game/ (NW.js used a privileged origin too, which file:// is not).
...
const webPreferences = {
  ...
  nodeIntegration: true, // NW.js games expect require() and process in the page
  contextIsolation: false,
  sandbox: false,
  webSecurity: false, // NW.js allowed file:// and cross-origin loads too
  backgroundThrottling: false,
  spellcheck: false,
};

// BrowserWindow options from an NW.js window manifest (package.json "window", or nw.Window.open()).
...
const ops = {
  ...
  close: (w, force) => { setImmediate(() => (force ? w.destroy() : w.close())); }, // force skips 'close' hooks
...
// Only structured-cloneable values can go back to the page over sendSync; anything else
// (a timer, a promise, an Electron object) would throw "An object could not be cloned".
...
// ipcRenderer.sendSync('nw', targetWindowId | null, op, ...args); null = the caller's window
...
// Send an nw.Window event to the window itself and to the window that opened it.
function emit(w, ev) {
...
app.on('browser-window-created', (_e, w) => {
  w.removeMenu();
  w.rpgmProps = {}; // properties games set on nw.Window objects, shared by all handles
  // NW.js semantics: a 'close' listener in the page decides whether the window closes.
  w.on('close', e => {
  ...
  w.on('closed', () => {
    ...
    // Games may hide their first window and continue in one they opened (e.g. SRD_GameUpgrade):
    // quit once nothing visible is left instead of leaving hidden windows running.
  ...
  // NW.js windows opened by the game (nw.Window.open / window.open) get Node.js too.
...
app.whenReady().then(() => {
  // No network traffic of our own: the spellchecker would otherwise download a dictionary.
  session.defaultSession.setSpellCheckerEnabled(false);
  session.defaultSession.setSpellCheckerLanguages([]);

  protocol.handle('app', req => {
    let file = null;
    try { file = ci.resolveIn(g.root, decodeURIComponent(new URL(req.url).pathname)); } catch { /* bad URL */ }
    if (!file || !fs.statSync(file).isFile()) return new Response('Not found', { status: 404 });
    const range = req.headers.get('range'); // audio/video seeking
